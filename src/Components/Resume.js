  ...
  render() {
    ...
    if(this.props.data){
      ...
      var work = this.props.data.work.map(function(work) {
        ...
        var arr = []
        for(const [index, value] of work.highlights.entries()) {
          arr.push(<i key={index}>{value}</i>)
        }
        // console.log('arr ', arr[0].props.children.highlight)
            ...
            <ul>
              {
                ...
                arr.map((value, index) => <li key={index}>&bull;{' '}{value.props.children.highlight}</li>)             
                )
              }
            </ul>
          </div>
